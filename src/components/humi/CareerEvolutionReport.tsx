...
import { buildCounselling } from "@/lib/humi/counselling";
...
interface Props {
  ...
  signup: SignupData;
  interestRole?: string;
  interestIndustry?: string;
  onRestart: () => void;
}

export function CareerEvolutionReport({ report, parsed, signup, interestRole, interestIndustry, onRestart }: Props) {
  const c = useMemo(
    () =>
      buildCounselling(
        parsed,
        { chosenRole: interestRole ?? report.futureRole, notes: "", industry: interestIndustry ?? parsed.industryExposure },
        signup,
        report,
      ),
    [parsed, signup, report, interestRole, interestIndustry],
  );
  ...
  return (
    <div className="px-5 py-10">
      <div className="mx-auto max-w-6xl">
        ...
        <Section n={1} title="Your Career Starting Point">
          <div className="surface-card p-6">
            ...
            <p className="mt-4 rounded-2xl bg-tint p-4 text-sm leading-relaxed">
              ...
              your resume shows clear signals in {parsed.skills.slice(0, 3).join(", ").toLowerCase()} within{" "}
              {parsed.primaryFunction.toLowerCase()}, with {parsed.yearsExperience} of exposure to{" "}
              {parsed.industryExposure}. Those signals map most directly to {report.futureRole} work, where your
              existing strengths stay valuable and AI tools remove the repetitive parts.
            </p>
            <div className="mt-5 grid gap-5 sm:grid-cols-2">
              <div>
                <p className="text-sm font-extrabold">Current strengths</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {report.strengths.map((s) => <span key={s} className="pill-tag">{s}</span>)}
                </div>
              </div>
              <div>
                <p className="text-sm font-extrabold">Areas with the biggest upside</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {report.gaps.length ? report.gaps.map((g) => <span key={g} className="pill-tag">{g}</span>) : <span className="pill-tag">Keep building depth</span>}
                </div>
              </div>
            </div>
          </div>
        ...
        <Section n={3} title="How to Improve Your Resume" subtitle="Specific, practical changes you can make this week.">
        ...
        <Section n={4} title="Your Role Evolution">
          <div className="grid items-stretch gap-5 md:grid-cols-[1fr_auto_1fr]">
            <div className="surface-card p-6">
              <p className="text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground">Current profile</p>
            ...
            <div className="tint-card p-6">
              <p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Future AI-enabled role</p>
          ...
          <div className="surface-card mt-5 p-6">
            ...
            <div className="mt-4 space-y-3">
              {c.dailyWork.map((d) => (
                <div key={d.before} className="grid gap-2 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
        ...
        <Section n={5} title="Roles You Can Start Targeting" subtitle="Realistic job titles based on your resume signals and interest.">
          <TargetRoles roles={c.targetRoles} />
        </Section>

        <Section n={6} title="AI Impact Map" subtitle="AI will change the task mix inside your role — your human skills become more valuable.">
        ...
        <Section n={8} title="What to Learn First" subtitle="A simple priority matrix so you never have to guess where to start.">
          <SkillPriorityMatrix matrix={c.matrix} />
        </Section>

        <Section n={9} title="Skills You Should Learn Next" subtitle="Start with the top five. Open the groups below only when you are ready for more.">
          <div className="tint-card p-6">
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Top 5 priority skills</p>
          ...
          <Accordion type="single" collapsible className="mt-5 space-y-3">
            {report.skillGroups.map((group, gi) => (
              <AccordionItem key={group.title} value={`g${gi}`} className="surface-card border-none px-5">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {group.skills.map((s) => (
                      <div key={s.name} className="tint-card p-5">
                        ...
                        <p className="mt-2 text-sm"><span className="font-semibold">Start with: </span><span className="text-muted-foreground">{s.beginnerAction}</span></p>
        ...
        <Section n={10} title="Recommended AI Tools to Explore" subtitle="Grouped so you know exactly which tool to open first.">
          <ToolRecommendations groups={c.toolGroups} />
        </Section>

        <Section n={11} title="Mini Projects You Can Build to Prove Your AI Readiness" subtitle="This is a practical way to prove your readiness — finish one, then talk about it in interviews.">
          <PortfolioProjects projects={c.projects} />
        </Section>

        <Section n={12} title="Your 30-60-90 Day Career Learning Path" subtitle="Each phase ends with something tangible you can show.">
          <LearningPath path={report.path} outputs={c.pathOutputs} />
        </Section>

        <Section n={13} title="Career Opportunity Score" subtitle="Here is how each score is calculated and what raises it.">
        ...
        <Section n={15} title="Keywords to Use in Your Job Search" subtitle="Use these in job searches, applications and your LinkedIn profile.">
          <JobSearchKeywords groups={c.keywords} />
        </Section>

        <Section n={16} title="How to Use AI Professionally" subtitle="Simple habits that keep your AI use safe, honest and credible.">
        ...
        <Section n={18} title="Future Resume Bullets" subtitle="Some you can use today. Others you earn by finishing a project.">
          <div className="grid gap-5 md:grid-cols-2">
            <div>
              <p className="text-sm font-extrabold text-primary">Bullets you can use now</p>
              <div className="mt-3 space-y-3">
                {c.bulletsNow.map((b) => (
                  <div key={b} className="surface-card p-5 text-sm font-semibold">{b}</div>
                ))}
              </div>
            </div>
            <div>
              <p className="text-sm font-extrabold text-primary">Bullets you can earn after your projects</p>
              <div className="mt-3 space-y-3">
        ...
        <Section n={19} title="Your Next 7 Days" subtitle={`A short, personalized plan for a ${report.futureRole} track.`}>
        ...
        <div className="mt-14 border-t border-border pt-6 text-center">
          <button onClick={onRestart} className="rounded-full border border-[var(--color-border-soft)] bg-tint px-6 py-3 text-sm font-bold text-primary">
